...
Initial three-dimensional molecular geometries were generated from SMILES strings using RDKit. Explicit hydrogen atoms were added, followed by conformer generation via distance geometry. A preliminary geometry optimisation was performed using the Universal Force Field (UFF) to obtain physically reasonable starting structures for quantum mechanical calculations.
...
Each molecule was processed individually, producing standard MOPAC output (.out) and archive (.arc) files. Calculations that did not converge successfully were excluded from descriptor extraction.
